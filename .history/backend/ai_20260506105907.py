...
def _simulate_hand(
    ...
    player_value = player_ranks.copy()
    dealer_value = [dealer_upcard]
    left_cards = remaining_deck.copy()
    
    if (action == 'hit'):
        player_ranks.append(_draw_random(remaining_deck))
        if (is_bust):
            return -1

    if (action == 'stand'):
        dealer_value.append(_draw_random(left_cards))

        while hand_score(dealer_value)[0] < 17:
